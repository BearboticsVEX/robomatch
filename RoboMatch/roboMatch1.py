...
class MainApplication(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        term = 3 #init
        v = StringVar()
        v.set("F16") # initialize
        
        pd = IntVar()
        pd.set = 1 # initialize
        
        for text, term in TERMS:
            b = Radiobutton(master, text=text, value=text, variable=v,
                            command=ShowChoice, relief=SUNKEN)
            b.pack(side="left")
        
        
        for periodLabel, periodVal in [("1", 1), ("2", 2), ("3", 3), ("8", 8)]:
            p = Radiobutton(master, text=periodLabel, value=periodVal, variable=pd,
                            command=ShowChoice(self))
            p.pack()    
    def ShowChoice(self):
        print ("your choice is",v.get(), pd.get())
